...
CBaseEntity * CBaseEntity::Create( char *szName, const Vector &vecOrigin, const Vector &vecAngles, edict_t *pentOwner, int weaponFlags, int spawnFlags ) { return NULL; }
...
Vector UTIL_VecSkew( const Vector &vecSrc, Vector vecAiming, float rightOffset, edict_t *shooter ) { return 0; }
...
void CBasePlayer::ApplyWeaponPushback(float) { };
...
Vector CBasePlayer::GetAimForwardWithOffset( bool degrees ) { return Vector(); }
...
BOOL CBasePlayerWeapon :: AddPrimaryAmmo( int iCount, char *szName, int iMaxClip, int iMaxClip2, int iMaxCarry ) { return TRUE; }
